...
func (ExampleModel Models) ViewSeqDiag(View ViewSeqDiagtask) SeqDiagView {
	...
	sqlStatement3 := "SELECT  tbl_sequence_diagram.id_dataflow_structure, tbl_member_belongto_project.creator FROM tbl_sequence_diagram " +
		"INNER JOIN tbl_member_belongto_project ON tbl_sequence_diagram.creator = tbl_member_belongto_project.id " +
		"WHERE tbl_sequence_diagram.id_dataflow_Structure=$1 "
