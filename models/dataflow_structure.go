...
type DataflowViewtask struct {
	Email       string `json:"email"`
	Id_dataflow int    `json:"id_dataflow"`
}

type DataflowView struct {
	DataflowView []DataflowViewtask `json:"dataflow_userjour_view"`
}
...
func (ExampleModel Models) ViewDflow(View DataflowViewtask) DataflowView {

	sqlStatement3 := "SELECT tbl_dataflow.id, tbl_member_belongto_project.id_user  FROM tbl_dataflow_structure " +
		"INNER JOIN tbl_member_belongto_project ON tbl_dataflow_structure.creator = tbl_member_belongto_project.id " +
		"INNER JOIN tbl_dataflow ON tbl_dataflow_structure.id_dataflow = tbl_dataflow.id " +
